...
4. Writing C++ on the (Arduino) Teensy and Python on my computer to get something working for the first time (for example, preliminary code to generate hammer strike events and velocities).
5. Testing.

Step 2 - Analog Subsystem https://www.youtube.com/watch?v=p1Ko1ZMaAus

(note the audio was accidentally recorded mono channel)
...
5. Writing ADC SPI driver C++ code on the (Arduino) Teensy.
6. Testing.
...
2. Implement the algorithm in C++ on the (Arduino) Teensy.
...
4. Continue C++ development for the (Arduino) Teensy.
...
2. Modify the (Arduino) Teensy C++ code to include a lower layer that abstracts the hardware.
...
6. Continue improving the (Arduino) Teensy (HSP) C++ code and computer Python code.
7. Test.
...
11. Changes to (Arduino) Teensy C++ code to workaround the write time issue.
...
1. C++ on TPS ((Arduino) Teensy 4.1) - real-time data processing.
